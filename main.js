        ...
        if (event.target.tagName == "TD" && event.target.closest("tbody").id == "scanned-places") {
            for(let p of scannedPlaces)
            {
                if(p.name == event.target.textContent) {
                    ...
                    document.getElementById(p.featureID).setAttribute("stroke-width", 7);
                    return;
                }
            }
        }
...
(function() {
    ...
    //TODO: add option to get center coord of polygon area
    //TODO: add regex to remove any symbols such as ".", "-", ";", ","; "/" in both google results and waze
    ...
    function InitializeScriptTab()
    {
        ...
        td,th {
         ...
         <label for="radius">Enter Nearby Search radius (in m)</label>
         <input name="radius" id="radius_number" type="number" value="50"><br>

         <label for="residential"> Show residential places </label>
         <input name="residential" type="checkbox" id="residential_enable"><br>

         <label for="show_bounding_box">Show bounding box areas</label>
         <input name="show_bounding_box" type="checkbox" id="bbox_enable"><br>

         <label for="closed"> Show closed places (red: confirmed, orange: maybe)</label>
         <input name="closed" type="checkbox" id="closed_enable"/><br>
            ...
            if(!p.bbox && p.display) {
                ...
                service.findPlaceFromQuery(request, function(results, status) {
                    if (status === google.maps.places.PlacesServiceStatus.OK) {
                        ...
                        if(debug) console.log(results);
                        for (var i = 0; i < results.length; i++) {
                            if(true) console.log(`[google_places_api] Fetched results for findPlace query '${p.name + p.fullAddress}' === ${results[i].name} ( ${results[i].formatted_address} )`);
                            if(checkSimilarity(results[i], p)) {
                                match = true;
                                console.log("Found similarity");
                    ...
                    if(!match && !p.closed) {
                        ...
                        service.nearbySearch(request, function(results, status) {
                            if (status === google.maps.places.PlacesServiceStatus.OK) {
                                ...
                                if(debug) console.log(results);
                                for (var i = 0; i < results.length; i++) {
                                    if(debug) console.log(`[google_places_api] Second try: Fetched results for nearby search query '${p.coords.join(",")}' => matched ${results[i].name}`);
                                    if(checkSimilarity(results[i], p)) {
                                        match = true;
    ...
    function displayPlaces()
    {
        ...
        for(let place of scannedPlaces) {
            ...
            if(place.node != null) {
                ...
                if(!options.closed) {
                    place.node.children[0].style.color = 'black';
                    //remove circle overlays
                    if(place.circleOverlay)
                        document.getElementById(place.parentFeatureID).removeChild(place.circleOverlay);
                }
            }
